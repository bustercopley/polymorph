...
void cramer::inverse (const float (& m) [3] [4], float (& minv) [3] [4])
{
  ...
  store4f (minv [0], rdet * _mm_shuffle_ps (c0t, c2s, SHUFFLE (2, 3, 1, 3)));
  store4f (minv [1], rdet * _mm_shuffle_ps (c1t, c2s, SHUFFLE (0, 1, 2, 3)));
  store4f (minv [2], rdet * _mm_shuffle_ps (c0t, c2s, SHUFFLE (0, 1, 0, 3)));
}
