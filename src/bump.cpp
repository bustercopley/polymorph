...
{
  // Evaluate the polynomial f. Return
  //   (0 0 0 0)  if t < t0,
  //   (f f f f)  if t0 <= t < t1,
  //   (1 1 1 1)  if t > t1.
  v4f c4 = load4f (c);
  ...
  const bump_specifier_t & b0, const bump_specifier_t & b1)
{
  ...
  v4f b0t = load4f (& b0.t0); // b0.t0 b0.t1 b0.t2 b0.t3
  v4f b1t = load4f (& b1.t0); // b1.t0 b1.t1 b1.t2 b1.t3
...
{
  // Compute all four polynomials, and mask and combine the values
  // according to the region of the graph to which t belongs.
  v4f s = _mm_set1_ps (t);
  v4f c0 = load4f (c [0]);
  v4f c1 = load4f (c [1]);
  v4f c2 = load4f (c [2]);
  v4f c3 = load4f (c [3]);
  v4f f = ((c3 * s + c2) * s + c1) * s + c0;
  v4f ltS = _mm_cmplt_ps (s, load4f (S0));
  v4f geT = _mm_cmpge_ps (s, load4f (T0));
  v4f x1 = _mm_andnot_ps (_mm_or_ps (ltS, geT), f);
  v4f x2 = _mm_and_ps (ltS, load4f (U0));
  v4f x3 = _mm_and_ps (geT, load4f (V0));
